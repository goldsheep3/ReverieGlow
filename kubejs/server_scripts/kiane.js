const update_history = [
  ...
  { version: "b1.3.2", desc: "[命令]增加了查询更新历史的/kiane_update_history" },
...
PlayerEvents.loggedIn(event => {
  ...
  // 获取需要显示的更新（使用filter更简洁）
  const updatesToShow = versions.filter(v => !player.stages.has(v));
  ...
  if (updatesToShow.length > 0) {
    ...
    if (!player.stages.has('kiane_beforetomorrow_joined')) {
      player.stages.add('kiane_beforetomorrow_joined');  // 标记新玩家，下次视为老玩家
      versions.forEach(v => player.stages.add(v));  // 标记所有版本为已完成
      player.tell("§b祝你游玩愉快！");
    } else {
      // 老玩家显示更新内容
      player.tell("§b更新内容:");
