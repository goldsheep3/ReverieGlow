const UPDATE_HISTORY = [
  ...
  { version: "0.3.2b", desc: "启用了两个优化模组" },

  // todo
  // { version: "", desc: "模组整体更新" }
  // { version: "", desc: "矿石开掘块的处理方式" },
  // { version: "", desc: "机械动力使用设备虚空合成配方合成x2"},
...
function sendUpdateLog(player, updates, options) {
  ...
  list.forEach(function(item) {
    player.tell(prefix + " §6" + item.version + "§b: §a" + item.desc);
...
function tellVersionAndQQ(player) {
  player.tell(
    Text.of("§b当前版本为 §6" + newestVersion + " §b。遇到问题或建议可加QQ群: §e" + QQ_GROUP)
      .clickCopy(QQ_GROUP)
...
PlayerEvents.loggedIn(function(event) {
  ...
  player.tell("§3================= [reglow] =================");
  player.tell("§b感谢游玩 §2GoldSheep3 §b制作的休闲整合 §6朝暮拾光 §b！");
  tellVersionAndQQ(player);

  if (!player.stages.has('reglow_joined')) {
    player.stages.add('reglow_joined');
    ...
    if (moreCount > 0) {
      player.tell(
        Text.of("§b……§e" + moreCount + "§b个更多版本更新日志。加入QQ群§e" + QQ_GROUP + "§b获取更多信息。")
          .clickCopy(QQ_GROUP)
          .hover("§b点击复制群号: §e" + QQ_GROUP)
      );
    }
